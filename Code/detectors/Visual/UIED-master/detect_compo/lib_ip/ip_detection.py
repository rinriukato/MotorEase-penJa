...
Component = importlib.import_module("detectors.Visual.UIED-master.detect_compo.lib_ip.Component")
Config = importlib.import_module("detectors.Visual.UIED-master.config.CONFIG_UIED")
#from config.CONFIG_UIED import Config
C = Config.Config()
    ...
    for i in range(0, row, step_h):
        for j in range(i % 2, column, step_v):
            if binary[i, j] == 255 and mask[i, j] == 0:
                ...
                component = Component.Component(region, binary.shape)
    ...
    for x in range(0, row, step_h):
        for y in range(0, column, step_v):
            if mask[x, y] == 0:
                ...
                compo = Component.Component(region, grey.shape)
