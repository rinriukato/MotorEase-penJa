...
def resize_height_by_longest_edge(img_path, resize_length=800):
    ...
    if height > width:
        ...
        #print(int(resize_length * (height / width)))
...
if __name__ == '__main__':
    ...
    '''
    onlyfiles = [f for f in listdir(".../touchTarget/testSet/screens/") if isfile(join(".../touchTarget/testSet/screens/", f))]
    print(len(onlyfiles))
    key_params = {'min-grad':10, 'ffl-block':5, 'min-ele-area':50,
                      'merge-contained-ele':True, 'merge-line-to-paragraph':False, 'remove-bar':True}
    #os.chdir('.../UIED-master/data/input/')
    count = 0 
    for files in onlyfiles:
        path = ".../touchTarget/testSet/screens/"
        fileLoc = path+files
        # print('-----------------------')
        # print(fileLoc)
        try:
            # set input image path
            input_path_img = fileLoc
            output_root = '.../touchTarget/testSet/newUIED'
