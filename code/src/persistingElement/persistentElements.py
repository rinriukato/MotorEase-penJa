...
dataPath = ".../MIRACLE/Data/RealData"
...
def PersistentDriver(dataPath):
    for subdir, dirs, files in os.walk(dataPath):
        for indApp in dirs:
            ...
            screenshots = []
            xmls = []
            for subdir1, dirs1, files1 in os.walk(dataPath + "/" + indApp):
                for file in files1:
                    if ".png" in file: 
                        screenshots.append(file)
                    if ".xml" in file: 
                        xmls.append(file)
                        xmlPath = dataPath + "/" + indApp + "/" + file
    ...
    results = open(".../MIRACLE/results/PeristentEvaluationResults.txt", 'w')
    print(noViolations)
    print(allViolations)
